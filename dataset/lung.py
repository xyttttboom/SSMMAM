...
def get_lung_dataset(root, num_labels, transform_train=None, transform_val=None, transform_forsemi=None):

    path_train_data = glob.glob(root + 'myTraining_Data512/*.png')
    path_valid_data = glob.glob(root + 'myValid_Data512/*.png')
    path_test_data = glob.glob(root + 'myTest_Data512/*.png')
    ...
    if num_labels < 2000:
        a = np.loadtxt("data_id/lung_id"+str(num_labels)+".txt", dtype='str')
        a = [root + "myTraining_Data512/" + item for item in a]
    ...
    path_train_label = path_train_label = [item.replace("myTraining_Data512", "myTraining_Label512") for item in path_train_data]
    path_valid_label = path_train_label = [item.replace("myValid_Data512", "myValid_Label512") for item in path_train_data]
    path_test_label = path_train_label = [item.replace("myTest_Data512", "myTest_Label512") for item in path_train_data]
...
class lung_labeled(data.Dataset):
...
class lung_unlabeled(data.Dataset):
